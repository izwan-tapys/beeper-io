...
async function run() {
  const { data, error } = await supabase.from('merchants').select('id, name, plan_type, subscription_status, expiry_date, upsell_video_url').neq('upsell_video_url', null);
  console.log("Upsell URLs:", data);
}
run();
